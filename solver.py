...
# For all s, j, \sum_k x(s,k,j,1) + x(s,k,j,0) \leq 1. Guarantees that each slot is used at most once.
    for u in from_nodes:
        for v in helpers.get_slots_of_type(to_nodes, 0):
            x = helpers.get_alt_slot(v, prev_slot) #sees if slot is potential 4hr (has alternate node)
            if x != None:
...
def run_solver(student_nodes, slot_nodes, wt, df, slotdict, min_exp, min_skill, stress_slots, target_delta, flex_shifts, duration):
